...
func (g GithubRepoImpl) SelectRepoByName(context context.Context, name string) (model.GithubRepo, error) {
	...
	if err != nil {
		if err == sql.ErrNoRows {
			return repo, error_message.RepoNotFound
		}
		log.Error(err.Error())
		return repo, err
	}
	...
	if err != nil {
		if err, ok := err.(*pq.Error); ok {
			if err.Code.Name() == "unique_violation" {
				return repo, error_message.RepoConflict
			}
		}
		log.Error(err.Error())
		return repo, error_message.RepoInsertFail
	}
...
func (g GithubRepoImpl) UpdateRepo(context context.Context, repo model.GithubRepo) (model.GithubRepo, error) {
	...
	if err != nil {
		log.Error(err.Error())
		return repo, error_message.RepoNotUpdated
	}
	if count == 0 {
		return repo, error_message.RepoNotUpdated
	}
...
func (g GithubRepoImpl) SelectAllBookmarks(context context.Context, userId string) ([]model.GithubRepo, error) {
	...
	if err != nil {
		if err == sql.ErrNoRows {
			return repos, error_message.BookmarkNotFound
		}
		log.Error(err.Error())
		return repos, err
	}
...
func (g GithubRepoImpl) Bookmark(context context.Context, bid, nameRepo, userId string) error {
	...
	if err != nil {
		if err, ok := err.(*pq.Error); ok {
			if err.Code.Name() == "unique_violation" {
				return error_message.BookmarkConflic
			}
		}
		log.Error(err.Error())
		return error_message.BookmarkFail
	}
...
func (g GithubRepoImpl) DelBookmark(context context.Context, nameRepo, userId string) error {
	...
	if err != nil {
		log.Error(err.Error())
		return error_message.DelBookmarkFail
	}
